...
| `tools.py` | Single tool registry: `TOOL_TABLE` defines each tool exactly once as `{name, description, input_schema, handler}`; `FS_TOOLS` (Anthropic shape) and `OLLAMA_TOOLS` (OpenAI fn shape) are both generated from it. `_execute_tool` dispatches by name. Add a tool = add one table entry |
...
| `staging.py` | Gated writes: propose → human review → apply. Re-validates every guard at approve time. Records also carry quote-anchored reviewer `comments` (add/resolve) and a `revisions` history (`update_content`, pending-only, last 5 kept) — approve stays the only step that touches disk. Review routes live in `main.py` under `/staging…` |
